...
extension UITableView {
    func setHeightFull(sender: NSLayoutConstraint) {
        sender.constant = CGFloat.greatestFiniteMagnitude
        self.reloadData()
        self.layoutIfNeeded()
        sender.constant = self.contentSize.height
    }
}
...
extension UIView
{
    ...
    @IBInspectable var tLeftBRightRound: CGFloat {
        get {
            return 0.0
        }
        set {
            let path = UIBezierPath(roundedRect: bounds, byRoundingCorners: [.topLeft, .bottomRight], cornerRadii: CGSize(width: newValue, height: newValue))
            let mask = CAShapeLayer()
            mask.path = path.cgPath
            layer.mask = mask
        }
